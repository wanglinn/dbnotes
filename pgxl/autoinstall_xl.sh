...
#!/bin/bash
installPath=/home/wln/install/install_102
dataPath=/home/wln/software/pgxl/data
pgxcPortBase=5700
coordinatorNum=2
datanodeNum=2
pgxcIp=127.0.0.1
...
function stop()
{
	...
	while [ $i -lt `expr $datanodeNum + $startNumDN` ]
	do
	    $installPath/bin/pg_ctl stop -D $dataPath/datanode$i -m i -l logfile  > /dev/null 2>&1
		i=`expr $i + 1`
	done
	...
	while [ $i -lt `expr $coordinatorNum + $startNumCN` ]
	do
	    $installPath/bin/pg_ctl stop -D $dataPath/coordinator$i -m i -l logfile   > /dev/null 2>&1
		i=`expr $i + 1`
	done

	#stop gtm
	$installPath/bin/gtm_ctl stop -D $dataPath/gtm -Z gtm -m i -l logfile
}
...
function regist()
{
    ...
    registString=$registDatanodeString$registCoordinatorString
    i=$startNumCN
    coordinatorPort=$pgxcPortBase
    while [ $i -lt `expr $coordinatorNum + $startNumCN` ]
	do
    ...
    i=$startNumDN
    datanodePort=$coordinatorPort
    datanodePortTemp=$coordinatorPort
    while [ $i -lt `expr $datanodeNum + $startNumDN` ]
	do
	    j=$startNumDN
            registDatanodeString=""
            while [ $j -lt `expr $datanodeNum + $startNumDN` ]
	        do
		    if [ $i -eq $j ] ; then
	                registDatanodeString=$registDatanodeString"alter node datanode$j with(type=datanode,host='$pgxcIp',port=$datanodePortTemp);"
                    else
			registDatanodeString=$registDatanodeString"create node datanode$j with(type=datanode,host='$pgxcIp',port=$datanodePortTemp);"
                    fi
		    j=`expr $j + 1`
	            datanodePortTemp=`expr $datanodePortTemp + 2`
	        done

            echo $registDatanodeString$registCoordinatorString
	    $installPath/bin/psql -d postgres -p $datanodePort -c "$registDatanodeString$registCoordinatorString"            
	    i=`expr $i + 1`
	    datanodePort=`expr $datanodePort + 2`
	done
